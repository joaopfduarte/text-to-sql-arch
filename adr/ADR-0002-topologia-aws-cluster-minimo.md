...
O TCC precisa de um ambiente laboratorial reprodutível para executar o pipeline Text-to-SQL com catálogo real (Apache Atlas) e massa PS (92 tabelas). O legado OCI, agora em [`../legacy-infra/`](../legacy-infra/), é somente referência de migração e não alvo de implantação final.
...
- EBS gp3 mínimo de 100 GiB por nó do cluster (master e workers) para suportar a massa `putz_teste` fixa com
  replicação HDFS 3.
