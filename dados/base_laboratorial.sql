-- Dialeto: MySQL 8+
-- Database: base_laboratorial
-- Schema de laboratorio versionado em artifact-docs/dados/
-- Import: mysql -u root -p < base_laboratorial.sql

-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Generation Time: Jun 27, 2026 at 05:00 AM
...
CREATE ALGORITHM=TEMPTABLE SQL SECURITY DEFINER VIEW `ibge_names_and_frequency`  AS SELECT `n`.`first_name` AS `first_name`, `f`.`f_frequency` AS `f_frequency`, sum(if((`f`.`gender` = 'M'),`f`.`f_total`,0)) AS `f_male`, sum(if((`f`.`gender` = 'F'),`f`.`f_total`,0)) AS `f_female`, if((sum(if((`f`.`gender` = 'M'),`f`.`f_total`,0)) < sum(if((`f`.`gender` = 'F'),`f`.`f_total`,0))),'F',if((sum(if((`f`.`gender` = 'M'),`f`.`f_total`,0)) > sum(if((`f`.`gender` = 'F'),`f`.`f_total`,0))),'M','O')) AS `gender`, if((sum(if((`f`.`gender` = 'M'),`f`.`f_total`,0)) < sum(if((`f`.`gender` = 'F'),`f`.`f_total`,0))),(sum(if((`f`.`gender` = 'F'),`f`.`f_total`,0)) / sum(`f`.`f_total`)),if((sum(if((`f`.`gender` = 'M'),`f`.`f_total`,0)) > sum(if((`f`.`gender` = 'F'),`f`.`f_total`,0))),(sum(if((`f`.`gender` = 'M'),`f`.`f_total`,0)) / sum(`f`.`f_total`)),1)) AS `ratio` FROM (`base_laboratorial`.`ibge__name` `n` left join `base_laboratorial`.`ibge__name_frequency` `f` on((`n`.`id` = `f`.`name_id`))) GROUP BY `n`.`first_name`, `f`.`f_frequency` ;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `project_render_user`  AS SELECT `base_laboratorial`.`person`.`id` AS `person_id`, `base_laboratorial`.`project_render`.`id` AS `project_render_id`, `base_laboratorial`.`project_render`.`render_slug` AS `render_slug` FROM ((`base_laboratorial`.`project` join `base_laboratorial`.`person` on(((`base_laboratorial`.`project`.`client_id` = `base_laboratorial`.`person`.`id`) or `base_laboratorial`.`person`.`id` in (select `p`.`id` from `base_laboratorial`.`person` `p` where (`p`.`company_id` = `base_laboratorial`.`project`.`client_id`))))) join `base_laboratorial`.`project_render` on((`base_laboratorial`.`project_render`.`project_id` = `base_laboratorial`.`project`.`id`))) WHERE ((`base_laboratorial`.`person`.`is_active` = true) AND (`base_laboratorial`.`project_render`.`is_active` = true)) ;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `publications_complete`  AS SELECT `p`.`id` AS `id`, `p`.`publications_source_id` AS `publications_source_id`, `p`.`source_type` AS `source_type`, `p`.`platform` AS `platform`, `p`.`json_data` AS `json_data`, `p`.`url` AS `url`, `p`.`news_time` AS `news_time`, `s`.`published_at` AS `published_at`, `s`.`title` AS `title`, `s`.`main_image` AS `main_image`, group_concat(`t`.`name` order by `t`.`name` ASC separator ', ') AS `tags` FROM (((`base_laboratorial`.`publications` `p` left join `base_laboratorial`.`publications_source` `s` on((`s`.`id` = `p`.`publications_source_id`))) left join `base_laboratorial`.`publications_source__tags` `pt` on((`pt`.`publication_source_id` = `s`.`id`))) left join `base_laboratorial`.`tag` `t` on((`pt`.`tags_id` = `t`.`id`))) GROUP BY `p`.`id`, `p`.`publications_source_id`, `p`.`source_type`, `p`.`platform`, `p`.`json_data`, `p`.`url`, `p`.`news_time`, `s`.`published_at`, `s`.`title`, `s`.`main_image` ORDER BY `s`.`published_at` DESC, `s`.`id` DESC, `p`.`platform` ASC ;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `render_item_resume`  AS SELECT `base_laboratorial`.`project_render_item`.`id` AS `id`, `base_laboratorial`.`project_render_item`.`render_slug` AS `render_slug`, `base_laboratorial`.`project_render_item`.`render_status` AS `render_status`, `base_laboratorial`.`project_render_item`.`name` AS `NAME`, `base_laboratorial`.`project_render_item`.`description` AS `description`, `base_laboratorial`.`project_render_item`.`whatsapp` AS `whatsapp`, `base_laboratorial`.`project_render_item`.`data_source` AS `data_source`, `base_laboratorial`.`project_render_item`.`video_url` AS `video_url` FROM `base_laboratorial`.`project_render_item` ;
...
--
-- Indexes for table `persistent_audit_event`
--
...
--
-- Indexes for table `persistent_audit_evt_data`
--
...
--
-- AUTO_INCREMENT for table `persistent_audit_event`
--
ALTER TABLE `persistent_audit_event`
...
ALTER TABLE `asset_category__users`
  ...
  ADD CONSTRAINT `fk_asset_category_users__user_id` FOREIGN KEY (`person_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `publications_user`
  ...
  ADD CONSTRAINT `fk_publications_user_requester` FOREIGN KEY (`owner_id`) REFERENCES `user` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
--
ALTER TABLE `user_authority`
  ADD CONSTRAINT `fk_authority_name` FOREIGN KEY (`authority_name`) REFERENCES `authority` (`name`) ON DELETE RESTRICT ON UPDATE CASCADE,
...
--
ALTER TABLE `subscriptions`
  ADD CONSTRAINT `subscriptions_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE,
...
--
ALTER TABLE `timeline_comment`
  ADD CONSTRAINT `fk_timeline_comment__created_by` FOREIGN KEY (`created_by`) REFERENCES `user` (`login`) ON DELETE RESTRICT ON UPDATE CASCADE,
