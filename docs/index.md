...
Diagramas UML (PlantUML): fontes em `diagrams/*.puml`; SVG gerados por `scripts/render-diagrams.sh` (invocado automaticamente pelo `prepare-docs.sh`). Requer `plantuml`, `graphviz` e Java (ex.: `apt-get install -y plantuml graphviz default-jre-headless`). **Clique num diagrama** para abrir em tela cheia (lightbox com zoom).
