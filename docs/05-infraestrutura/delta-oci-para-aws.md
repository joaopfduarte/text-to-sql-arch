...
Listar as diferenças concretas entre o laboratório de referência (OCI ARM) e o alvo do TCC (AWS x86), com base em evidência de [`../legacy-infra/`](../legacy-infra/index.md) e no VDF canônico [`../assets/scripts/ODP-VDF.xml`](../assets/scripts/ODP-VDF.xml).
...
| Conflito documental legado | README legado tinha `/16`; `network.tf` usa `/24` | `/24` é a referência correta | [`index.md`](../legacy-infra/index.md), [`network.tf`](../legacy-infra/network.tf) |
...
| Sizing | 1 OCPU / 6 GiB (legado) | Master e workers `m6i.2xlarge` (8 vCPU, 32 GiB) | [`variables.tf`](../legacy-infra/variables.tf), [`../adr/ADR-0002-topologia-aws-cluster-minimo.md`](../adr/ADR-0002-topologia-aws-cluster-minimo.md) |
...
| Armazenamento | Boot 50 GiB | EBS gp3 >= 100 GiB por nó do cluster | `compute.tf` (legado), ADR-0002 |
...
7. Carregar PS conforme [`../04-arquitetura-dados/carga-cluster-putz.md`](../04-arquitetura-dados/carga-cluster-putz.md).
