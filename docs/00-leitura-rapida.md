...
Um pipeline Text-to-SQL onde o agente LLM **consulta a camada semântica e de metadados via MCP antes de gerar SQL**. No experimento, essa camada materializa-se como catálogo canônico (Apache Atlas). O objetivo é medir aderência estrutural, executabilidade e rastreabilidade contra um banco real (subconjunto PS, 92 tabelas) em cluster AWS mínimo.
...
| PS | Schema relacional de 92 tabelas; subconjunto fixo é a massa de avaliação. |
