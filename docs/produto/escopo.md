...
Um pipeline Text-to-SQL em que um agente LLM consulta a camada semântica e de metadados via servidor MCP antes de gerar SQL, valida a aderência estrutural contra o catálogo canônico e executa o SQL sob ambiente controlado, registrando trilha completa por corrida. O catálogo canônico é implementado com Apache Atlas (escolha arquitetural; ver [Catálogo Atlas](../arquitetura/dados/catalogo-atlas.md)).
